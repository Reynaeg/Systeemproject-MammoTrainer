...
We also chose Gradio as our package to create our application with. Gradio's documentation is sadly a little iffy, and we've experienced lots of bugs and unintuitive methods of creating certain functions. Gradio does not allow for simple annotations on an image, such as clicking and moving your mouse around to create a circle (similar to Microsoft Paint's circle tool), 
instead opting for methods like pressing 3 times to select coördinates to create a circle. Although it works fine enough for this prototype, for future projects we probably would not use Gradio but find an alternative way of creating our application.

There are a few small bugs left in the programme, such as ellipse results being saved (but not the visible ellipses on the images themselves) once the check button is pressed, the programme is exited and re-entered and the check button is pressed again. Overal, MammoTrainer should work pretty well.
